...
public class JsonHelper {
    ...
    public static String getValueJsonObject(String... keys) {
        ...
        try {
            JsonNode node = objectMapper.readTree(new File(DataConfig.JSON_DEVICE_FILE_PATH));
    ...
    public static String getValueJsonArray(int itemIndex, String... keys) {
        ...
        try {
            JsonNode rootNode = objectMapper.readTree(new File(DataConfig.JSON_DEVICE_FILE_PATH));
    ...
    public static void updateValueJsonObject(String keyName, Number value) {
        Reader reader;
        try {
            reader = Files.newBufferedReader(Paths.get(DataConfig.JSON_DEVICE_FILE_PATH));

            Gson gson = new Gson();
            //Convert Json file to Json Object
            JsonObject jsonObject = gson.fromJson(reader, JsonObject.class);
            LogHelper.info("Original JSON: " + jsonObject);

            //Update value if exist key
            jsonObject.addProperty(keyName, value);

            LogHelper.info("Modified JSON: " + jsonObject);

            //Store new Json data to file
            File jsonFile = new File(DataConfig.JSON_DEVICE_FILE_PATH);
    ...
    public static void updateValueJsonObject(String keyName, String value) {
        ...
        try {
            ...
            if (jsonObject.has(keyName)) {
                LogHelper.info("🔁 Update key: '" + keyName + "'");
            } else {
                LogHelper.info("➕ Add new key: '" + keyName + "'");
            }
    ...
    public static void updateValueJsonArray(String value, int index, String... keys) {
        ...
        try (Reader reader = Files.newBufferedReader(Paths.get(DataConfig.JSON_DEVICE_FILE_PATH))) {
            Gson gson = new Gson();
            ...
            // Ghi lại file
            try (OutputStream outputStream = new FileOutputStream(DataConfig.JSON_DEVICE_FILE_PATH)) {
                outputStream.write(gson.toJson(jsonArray).getBytes());
                outputStream.flush();
            }

            LogHelper.info("✅ Updated index " + index + ": " + String.join(" → ", keys) + " = " + value);
